...
async function createAADApp(appName, apiPermissions) {
    ...
    let createdApp = null;
    try {
        createdApp = JSON.parse(await $`m365 aad app add --name ${appName} --apisDelegated ${apiPermissions}  --redirectUris 'https://login.microsoftonline.com/common/oauth2/nativeclient' --platform publicClient --grantAdminConsent --output json`);
